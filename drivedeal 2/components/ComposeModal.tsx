    ...
    <div className="overlay" onClick={e => { if(e.target===e.currentTarget) onClose() }}>
      <div className="modal modal-lg">
        <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:20}}>
          <div>
            <div className="font-head" style={{fontSize:17,fontWeight:700,letterSpacing:'-0.3px'}}>AI Email Composer</div>
        ...
        <div style={{background:'var(--surface2)',border:'1px solid var(--border)',borderRadius:9,padding:14,display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:16}}>
          {[['Navn',lead.name,'var(--text)'],['Email',lead.email,'var(--blue)'],['Bil interesse',lead.car,'var(--text)'],['Sidst kontaktet',`${lead.days} dage siden`,'var(--amber)'],['AI score',`${lead.score}/100`,scoreColor],['Status',lead.status,'var(--text)']].map(([l,v,c])=>(
            <div key={l as string}><div style={{fontSize:10,color:'var(--text3)',textTransform:'uppercase',letterSpacing:'.8px'}}>{l}</div><div style={{fontSize:13,fontWeight:500,marginTop:2,color:c as string}}>{v}</div></div>
          ))}
        </div>
        ...
        <div
          ...
          onInput={e => setBody((e.target as HTMLDivElement).innerText)}
        ...
        <div style={{display:'flex',justifyContent:'flex-end',gap:8,marginTop:22,paddingTop:16,borderTop:'1px solid var(--border)'}}>
          ...
          <button className="btn btn-gold" onClick={send} disabled={loading || !body}>
            <svg width="12" height="12" viewBox="0 0 16 16" fill="none"><path d="M1 1l14 7-14 7V9.5l10-2.5-10-2.5V1z" fill="currentColor"/></svg>
            Send via Gmail
          </button>
        </div>
      </div>
