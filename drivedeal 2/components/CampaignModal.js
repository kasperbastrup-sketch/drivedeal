...
export default function CampaignModal({ onClose, onLaunch }) {
  const [name, setName] = useState('Efterår Reactivation 2025')
  const [segIdx, setSegIdx] = useState(0)

  const seg = segments[segIdx]
  const opened  = Math.round(seg.count * 0.43)
  const booked  = Math.round(seg.count * 0.07)
  const revenue = (booked * 12000).toLocaleString('da')
    ...
    <div className="overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal">
        <div className="modal-header">
          <div>
            <div className="modal-title">Start ny AI kampagne</div>
            <div className="modal-sub">AI genererer og sender personaliserede emails til dit segment</div>
          </div>
          <button className="close-btn" onClick={onClose}>✕</button>
        </div>

        <div className="label">Kampagnenavn</div>
        ...
        <select className="field-select" style={{width:'100%'}} value={segIdx} onChange={e=>setSegIdx(Number(e.target.value))}>
          {segments.map((s,i) => <option key={i} value={i}>{s.label}</option>)}
        </select>
        <div style={{fontSize:12,color:'var(--text2)',marginTop:6}}>
          Estimeret målgruppe: <strong style={{color:'var(--gold)'}}>{seg.count} leads</strong>
        </div>
        ...
        <div style={{background:'var(--goldglow)',border:'1px solid rgba(201,169,110,.2)',borderRadius:9,padding:14,marginTop:16}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8,textAlign:'center'}}>
            <div><div style={{fontFamily:'var(--font-head)',fontSize:18,fontWeight:800,color:'var(--gold)'}}>{opened}</div><div style={{fontSize:10,color:'var(--text2)'}}>Åbner (43%)</div></div>
            <div><div style={{fontFamily:'var(--font-head)',fontSize:18,fontWeight:800,color:'var(--green)'}}>{booked}</div><div style={{fontSize:10,color:'var(--text2)'}}>Bookinger (7%)</div></div>
            <div><div style={{fontFamily:'var(--font-head)',fontSize:18,fontWeight:800,color:'var(--gold)'}}>€ {revenue}</div><div style={{fontSize:10,color:'var(--text2)'}}>Est. omsætning</div></div>
          </div>
        </div>
