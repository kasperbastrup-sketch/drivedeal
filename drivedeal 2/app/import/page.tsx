    ...
    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}}>
      <div>
        <div className="panel" style={{marginBottom:14}}>
          ...
          <div
            className="drop-zone"
            style={{borderColor: dragging ? 'var(--gold)' : undefined, background: dragging ? 'var(--goldglow)' : undefined}}
        ...
        <div className="panel">
          ...
          {[['Fuldt navn','text','Carlos Mendez'],['Email','email','carlos@gmail.com'],['Telefon','tel','+34 612 345 678'],['Bil interesse','text','BMW 520d, Mercedes GLC']].map(([l,t,p])=>(
            <div key={l as string}>
              <div className="label">{l}</div>
              <input className="field-input" type={t as string} placeholder={p as string} style={{width:'100%'}}/>
            </div>
          ))}
      ...
      <div>
        <div className="panel" style={{marginBottom:14}}>
          ...
          {[
            ...
            <div key={item.name} className={`onboard-step${item.done?' done':''}`}>
              <div style={{width:28,height:28,borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,fontWeight:700,flexShrink:0,border:`2px solid ${item.done?'var(--green)':'var(--border2)'}`,color:item.done?'var(--green)':'var(--text2)',background:item.done?'var(--greenbg)':'none'}}>
                {item.done?'✓':null}
              </div>
              <div style={{flex:1}}>
              ...
              {item.done
                ? <span className="pill pill-green">Aktiv</span>
                : <button className="btn btn-ghost btn-sm" onClick={()=>connectCRM(item.name)}>Forbind</button>
              }
            </div>
          ))}
        </div>
        ...
        <div className="panel">
          <div className="font-head" style={{fontSize:13,fontWeight:600,marginBottom:4}}>Auto-import regler</div>
          <div style={{fontSize:11,color:'var(--text2)',marginBottom:14}}>Bestem automatisk hvilke leads der importeres</div>
