...
const monthly = [
  {m:'Feb',rev:'€42k',sales:5,c:'var(--text2)'},
  {m:'Mar',rev:'€67k',sales:8,c:'var(--green)'},
  {m:'Apr',rev:'€88k',sales:11,c:'var(--green)'},
  {m:'Maj',rev:'€112k',sales:14,c:'var(--green)'},
  {m:'Jun',rev:'€134k',sales:16,c:'var(--green)'},
  {m:'Jul',rev:'€94k',sales:9,c:'var(--gold)',highlight:true},
]
    ...
    <div>
        ...
        <div className="panel">
          ...
          {[['Personaliseret',87,'var(--green)','58%'],['Bil-specifik',65,'var(--gold)','43%'],['Nysgerrig',52,'var(--amber)','35%'],['Tilbud',38,'var(--blue)','25%'],['Generisk',18,'var(--text3)','12%']].map(([l,p,c,v])=>(
            <div key={l as string} style={{display:'flex',alignItems:'center',gap:10,marginBottom:9}}>
              ...
              <div style={{fontSize:11,fontWeight:600,fontFamily:'var(--font-mono)',width:32,textAlign:'right',color:c as string}}>{v}</div>
            </div>
          ))}
        </div>
        <div className="panel">
      ...
      <div className="panel">
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(6,1fr)',gap:10,textAlign:'center'}}>
          {monthly.map(m=>(
            <div key={m.m} style={m.highlight?{background:'var(--goldglow)',borderRadius:8,border:'1px solid rgba(201,169,110,.3)',padding:'8px 4px'}:{padding:'8px 4px'}}>
              <div style={{fontSize:10,color:m.highlight?'var(--gold)':'var(--text3)',marginBottom:6}}>{m.m}{m.highlight?' ←':''}</div>
              <div className="font-head" style={{fontSize:16,fontWeight:700,color:m.highlight?'var(--gold)':'var(--text)'}}>{m.rev}</div>
